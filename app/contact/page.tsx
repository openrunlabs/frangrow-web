...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!agreed) { setErrorMsg("개인정보 수집 및 이용 동의가 필요합니다."); setShowError(true); return; }
    setIsSubmitting(true);
    const sols: string[] = [];
    form.querySelectorAll<HTMLInputElement>(".sol-cb:checked").forEach(cb => sols.push(cb.value));
    const payload = {
      name: fd.get("name"), role: fd.get("role"), phone: fd.get("phone"),
      company_name: fd.get("company_name"), industry: fd.get("industry"),
      content: fd.get("content"), target_solutions: sols, ...trackingData,
    };
    try {
      const res = await fetch("/api/contact", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      if (res.ok) { setShowSuccess(true); form.reset(); }
      else { const d = await res.json(); throw new Error(d.error || "전송 실패"); }
    } catch (err: unknown) {
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="flex items-center justify-between h-16 sm:h-20">
            ...
            <div className="flex items-center space-x-6 text-sm font-bold">
              <button onClick={() => router.push("/")} className="hidden sm:inline-block hover:text-[#D0190F] transition-colors">HOME</button>
              <button onClick={() => router.push("/services")} className="hover:text-[#D0190F] transition-colors">서비스 소개</button>
      ...
      <main className="py-16 sm:py-24">
        <div className="max-w-3xl mx-auto px-5 sm:px-8 space-y-12">
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-zinc-500 max-w-2xl mx-auto leading-relaxed">프차그로우는 현재 브랜드의 가맹확장, 검색 노출, 성장자금, AI 본부 시스템 가능성을 기준으로 실행 가능한 다음 단계를 진단합니다.</p>
          </div>

          <div className="bg-[#FAFAFA] border border-[#EBEBEB] p-8 sm:p-12 rounded-[18px] shadow-md">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div><label className="block text-xs font-bold text-[#111111] mb-2">이름 *</label><input required type="text" name="name" className="w-full text-sm p-4 bg-white border border-[#EBEBEB] rounded-[18px] focus:outline-none focus:border-[#D0190F]" placeholder="김성장 대표" /></div>
...
/>
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-white p-8 rounded-[18px] max-w-md w-full text-center shadow-2xl space-y-6">
            <div className="w-16 h-16 bg-[#D0190F] text-white flex items-center justify-center rounded-full mx-auto text-2xl font-black">✓</div>
      ...
      {showError && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-white p-8 rounded-[18px] max-w-md w-full text-center shadow-2xl space-y-6">
            <div className="w-16 h-16 bg-zinc-100 text-[#D0190F] flex items-center justify-center rounded-full mx-auto text-2xl font-black">✕</div>
